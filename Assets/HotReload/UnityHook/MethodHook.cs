...
namespace MonoHook
{
    ...
    public unsafe class MethodHook
    {
        ...
        private void CheckMethod()
        {
            ...
            string methodName = string.Format("{0}.{1}", targetMethod.DeclaringType.Name, targetMethod.Name);
            if (targetMethod.IsAbstract)
                throw new Exception(string.Format("WRANING: you can not hook abstract method [{0}]", methodName));
            ...
            if(proxyMethod != null)
            {
                methodName = string.Format("{0}.{1}", proxyMethod.DeclaringType.Name, proxyMethod.Name);
                int codeSize = proxyMethod.GetMethodBody().GetILAsByteArray().Length;
                if (codeSize < minMethodBodySize)
                    UnityEngine.Debug.LogWarning(string.Format("WRANING: size of method body[{0}] is too short({1}), will random crash on IL2CPP release mode, please fill some dummy code inside", methodName, codeSize));
            }
#endif
        }
        ...
        private bool GetFunctionAddr()
        {
            ...
            {
                throw new Exception(string.Format("the addresses of target method {0} and replacement method {1} can not be same", targetMethod.Name, replacementMethod.Name) );
            }
