...
namespace MonoHook
{
    public static unsafe class HookUtils
    {
        ...
        public static string HexToString(void* ptr, int size, int offset = 0)
        {
            ...
            Func<IntPtr, string> formatAddr = (IntPtr addr__) => IntPtr.Size == 4 ? string.Format("0x{0}", (uint)addr__) : string.Format("0x{0}", (ulong)addr__);

            byte* addr = (byte*)ptr;

            StringBuilder sb = new StringBuilder(1024);
            sb.AppendLine(string.Format("addr:{0}", formatAddr(new IntPtr(addr))));
            ...
            while (true)
            {
                sb.Append(string.Format("\r\n{0}: ", formatAddr(new IntPtr(addr + count))));
                for (int i = 1; i < PRINT_COL_SIZE + 1; i++)
                {
                    if (count >= size)
                        goto END;

                    sb.Append(string.Format("{0}", *(addr + count)));
                    if (i % PRINT_SPLIT == 0)
                        sb.Append(" ");
        ...
        static void SetupFlushICacheFunc()
        {
            ...
            if (IntPtr.Size == 4)
            {
                ...
                flush_icache = Marshal.GetDelegateForFunctionPointer(new IntPtr(s_ptr_flush_icache_arm32), typeof(DelegateFlushICache)) as DelegateFlushICache;
            }
            else
            {
                ...
                flush_icache = Marshal.GetDelegateForFunctionPointer(new IntPtr(s_ptr_flush_icache_arm64), typeof(DelegateFlushICache)) as DelegateFlushICache;
            }
