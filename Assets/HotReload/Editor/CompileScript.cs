...
namespace ScriptHotReload
{
    ...
    public static class CompileScript
    {
        ...
        public static void CompileScriptToDir(string outputDir)
        {
            if(!IsIdle())
            {
                Debug.LogError(string.Format("当前编译状态:{0}, 不允许执行编译", compileStatus));
                return;
            }
            ...
            HotReloadUtils.RemoveAllFiles(outputDir);
            ...
            Debug.Log(string.Format("开始编译dll到目录: {0}", outputDir));
        ...
        static void Init()
        {
            {// install hook
                ...
                var miNew = typeof(CompileScript).GetMethod("TickCompilationPipeline", BindingFlags.NonPublic | BindingFlags.Static);
                var miReplace = typeof(CompileScript).GetMethod("TickCompilationPipeline_Proxy", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        static void EditorApplication_Update()
        {
            if(s_CompileRequested)
            {
                if(IsIdle())
                {
                    ...
                    {
                        Debug.LogError(string.Format("编译失败:{0}", compileStatus));
                    }
