        ...
        static void OnPlayModeChange(PlayModeStateChange mode)
        {
            switch (mode)
            {
                case PlayModeStateChange.EnteredPlayMode:
                    {
                        HotReloadUtils.RemoveAllFiles(HotReloadConfig.kTempScriptDir);

                        CompileScript.ResetCompileStatus();
                        string json = EditorPrefs.GetString(HotReloadConfig.kEditorScriptBuildParamsKey);
                ...
                case PlayModeStateChange.ExitingEditMode: // 退出编辑模式保存编译参数
                    {
                        ...
                        EditorPrefs.SetString(HotReloadConfig.kEditorScriptBuildParamsKey, json);
                        break;
                    }
